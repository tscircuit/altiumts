...
export function getSchematicGraphicRightEdge(record: AltiumRecord): number {
  const x = getSchematicCoordinate(record, { key: "LOCATION.X", fallback: 0 })
  switch (record.recordKind) {
    ...
    case "8":
      return x + getSchematicCoordinate(record, { key: "RADIUS", fallback: 1 })
    case "11":
    case "12":
      return Math.max(
        ...approximateAltiumArc({
          center: {
            x,
            y: getSchematicCoordinate(record, {
              key: "LOCATION.Y",
              fallback: 0,
            }),
          },
          radius: getSchematicCoordinate(record, {
            key: "RADIUS",
            fallback: 1,
          }),
    ...
    default:
      return Math.max(
        x,
        getSchematicCoordinate(record, { key: "CORNER.X", fallback: x }),
      )
  }
}
